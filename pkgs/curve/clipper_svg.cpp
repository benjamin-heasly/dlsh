...
public:
  ...
  bool SaveToFile(const string& filename, double scale = 1.0, int margin = 10, bool flipy = false)
  {
    ...
    cInt yrange = rec.bottom-rec.top;
    ...
    for (PolyInfoList::size_type i = 0; i < polyInfos.size(); ++i)
  {
	...
	double x = ((double)polyInfos[i].paths[j][0].X * scale + offsetX);
	double y = ((double)polyInfos[i].paths[j][0].Y * scale + offsetY);
	if (flipy) y = (yrange-y)+rec.top;
        file << " M " << x << " " << y;
...
static bool SaveToFile(const string& filename, Paths &ppg, double scale = 1.0, unsigned decimal_places = 0)
{
...
static bool LoadFromFile(Paths &ppg, const string& filename, double scale)
{
  //file format assumes: 
...
static void MakeRandomPoly(int edgeCount, int width, int height, Paths & poly)
{
  poly.resize(1);
...
int save_clipper_svg(Paths &ppg, const string &filename, float svg_scale, bool flipy)
{
  SVGBuilder svg;
  ...
  svg.AddPaths(ppg);
  svg.SaveToFile(filename, svg_scale, 0, flipy);
  return 1;
}

...
extern "C" { int Cgbase_Init(Tcl_Interp *interp); }
...
public:
  ...
  void init(void)
  {
    ...
    setfviewport(0, 0, 1, 1) ;
    setcolor(0);		// black (on white background)
    gbInitGevents();
    initialized = true;
  }
  ...
  {
    if (currentCG) currentCG->redraw();
    return TCL_OK;
  }


  CGWin(Tcl_Interp *interp, int X, int Y, int W, int H, const char*L=0) :
    Fl_Box(X,Y,W,H,L), interp(interp) {
    currentCG = this;

    frame = nullptr;		// share for now
  ...
  {
    if (gbuf) {
      if (GB_GBUF(gbuf)) free(GB_GBUF(gbuf));
      free(gbuf);
    }
